/****************************************************************************
** Meta object code from reading C++ file 'mediaplayer2.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.15.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include <memory>
#include "../../../src/mpris2/mediaplayer2.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'mediaplayer2.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.15.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_MediaPlayer2_t {
    QByteArrayData data[15];
    char stringdata0[166];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_MediaPlayer2_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_MediaPlayer2_t qt_meta_stringdata_MediaPlayer2 = {
    {
QT_MOC_LITERAL(0, 0, 12), // "MediaPlayer2"
QT_MOC_LITERAL(1, 13, 15), // "D-Bus Interface"
QT_MOC_LITERAL(2, 29, 22), // "org.mpris.MediaPlayer2"
QT_MOC_LITERAL(3, 52, 5), // "raise"
QT_MOC_LITERAL(4, 58, 0), // ""
QT_MOC_LITERAL(5, 59, 4), // "quit"
QT_MOC_LITERAL(6, 64, 5), // "Raise"
QT_MOC_LITERAL(7, 70, 4), // "Quit"
QT_MOC_LITERAL(8, 75, 8), // "CanRaise"
QT_MOC_LITERAL(9, 84, 7), // "CanQuit"
QT_MOC_LITERAL(10, 92, 12), // "HasTrackList"
QT_MOC_LITERAL(11, 105, 8), // "Identity"
QT_MOC_LITERAL(12, 114, 12), // "DesktopEntry"
QT_MOC_LITERAL(13, 127, 19), // "SupportedUriSchemes"
QT_MOC_LITERAL(14, 147, 18) // "SupportedMimeTypes"

    },
    "MediaPlayer2\0D-Bus Interface\0"
    "org.mpris.MediaPlayer2\0raise\0\0quit\0"
    "Raise\0Quit\0CanRaise\0CanQuit\0HasTrackList\0"
    "Identity\0DesktopEntry\0SupportedUriSchemes\0"
    "SupportedMimeTypes"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_MediaPlayer2[] = {

 // content:
       8,       // revision
       0,       // classname
       1,   14, // classinfo
      11,   16, // methods
       7,   82, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // classinfo: key, value
       1,    2,

 // signals: name, argc, parameters, tag, flags
       3,    0,   71,    4, 0x06 /* Public */,
       5,    0,   72,    4, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       6,    0,   73,    4, 0x0a /* Public */,
       7,    0,   74,    4, 0x0a /* Public */,
       8,    0,   75,    4, 0x0a /* Public */,
       9,    0,   76,    4, 0x0a /* Public */,
      10,    0,   77,    4, 0x0a /* Public */,
      11,    0,   78,    4, 0x0a /* Public */,
      12,    0,   79,    4, 0x0a /* Public */,
      13,    0,   80,    4, 0x0a /* Public */,
      14,    0,   81,    4, 0x0a /* Public */,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Bool,
    QMetaType::Bool,
    QMetaType::Bool,
    QMetaType::QString,
    QMetaType::QString,
    QMetaType::QStringList,
    QMetaType::QStringList,

 // properties: name, type, flags
       8, QMetaType::Bool, 0x00095401,
       9, QMetaType::Bool, 0x00095401,
      10, QMetaType::Bool, 0x00095401,
      11, QMetaType::QString, 0x00095401,
      12, QMetaType::QString, 0x00095401,
      13, QMetaType::QStringList, 0x00095401,
      14, QMetaType::QStringList, 0x00095401,

       0        // eod
};

void MediaPlayer2::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        auto *_t = static_cast<MediaPlayer2 *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->raise(); break;
        case 1: _t->quit(); break;
        case 2: _t->Raise(); break;
        case 3: _t->Quit(); break;
        case 4: { bool _r = _t->CanRaise();
            if (_a[0]) *reinterpret_cast< bool*>(_a[0]) = std::move(_r); }  break;
        case 5: { bool _r = _t->CanQuit();
            if (_a[0]) *reinterpret_cast< bool*>(_a[0]) = std::move(_r); }  break;
        case 6: { bool _r = _t->HasTrackList();
            if (_a[0]) *reinterpret_cast< bool*>(_a[0]) = std::move(_r); }  break;
        case 7: { QString _r = _t->Identity();
            if (_a[0]) *reinterpret_cast< QString*>(_a[0]) = std::move(_r); }  break;
        case 8: { QString _r = _t->DesktopEntry();
            if (_a[0]) *reinterpret_cast< QString*>(_a[0]) = std::move(_r); }  break;
        case 9: { QStringList _r = _t->SupportedUriSchemes();
            if (_a[0]) *reinterpret_cast< QStringList*>(_a[0]) = std::move(_r); }  break;
        case 10: { QStringList _r = _t->SupportedMimeTypes();
            if (_a[0]) *reinterpret_cast< QStringList*>(_a[0]) = std::move(_r); }  break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        {
            using _t = void (MediaPlayer2::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&MediaPlayer2::raise)) {
                *result = 0;
                return;
            }
        }
        {
            using _t = void (MediaPlayer2::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&MediaPlayer2::quit)) {
                *result = 1;
                return;
            }
        }
    }
#ifndef QT_NO_PROPERTIES
    else if (_c == QMetaObject::ReadProperty) {
        auto *_t = static_cast<MediaPlayer2 *>(_o);
        Q_UNUSED(_t)
        void *_v = _a[0];
        switch (_id) {
        case 0: *reinterpret_cast< bool*>(_v) = _t->CanRaise(); break;
        case 1: *reinterpret_cast< bool*>(_v) = _t->CanQuit(); break;
        case 2: *reinterpret_cast< bool*>(_v) = _t->HasTrackList(); break;
        case 3: *reinterpret_cast< QString*>(_v) = _t->Identity(); break;
        case 4: *reinterpret_cast< QString*>(_v) = _t->DesktopEntry(); break;
        case 5: *reinterpret_cast< QStringList*>(_v) = _t->SupportedUriSchemes(); break;
        case 6: *reinterpret_cast< QStringList*>(_v) = _t->SupportedMimeTypes(); break;
        default: break;
        }
    } else if (_c == QMetaObject::WriteProperty) {
    } else if (_c == QMetaObject::ResetProperty) {
    }
#endif // QT_NO_PROPERTIES
}

QT_INIT_METAOBJECT const QMetaObject MediaPlayer2::staticMetaObject = { {
    QMetaObject::SuperData::link<QDBusAbstractAdaptor::staticMetaObject>(),
    qt_meta_stringdata_MediaPlayer2.data,
    qt_meta_data_MediaPlayer2,
    qt_static_metacall,
    nullptr,
    nullptr
} };


const QMetaObject *MediaPlayer2::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *MediaPlayer2::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_MediaPlayer2.stringdata0))
        return static_cast<void*>(this);
    return QDBusAbstractAdaptor::qt_metacast(_clname);
}

int MediaPlayer2::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDBusAbstractAdaptor::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 11)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 11;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 11)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 11;
    }
#ifndef QT_NO_PROPERTIES
    else if (_c == QMetaObject::ReadProperty || _c == QMetaObject::WriteProperty
            || _c == QMetaObject::ResetProperty || _c == QMetaObject::RegisterPropertyMetaType) {
        qt_static_metacall(this, _c, _id, _a);
        _id -= 7;
    } else if (_c == QMetaObject::QueryPropertyDesignable) {
        _id -= 7;
    } else if (_c == QMetaObject::QueryPropertyScriptable) {
        _id -= 7;
    } else if (_c == QMetaObject::QueryPropertyStored) {
        _id -= 7;
    } else if (_c == QMetaObject::QueryPropertyEditable) {
        _id -= 7;
    } else if (_c == QMetaObject::QueryPropertyUser) {
        _id -= 7;
    }
#endif // QT_NO_PROPERTIES
    return _id;
}

// SIGNAL 0
void MediaPlayer2::raise()
{
    QMetaObject::activate(this, &staticMetaObject, 0, nullptr);
}

// SIGNAL 1
void MediaPlayer2::quit()
{
    QMetaObject::activate(this, &staticMetaObject, 1, nullptr);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
